...
memorizzato, deve essere diviso in blocchi da 512 byte ciascuno.
 
Si sviluppi in Python un algoritmo per questa funzione che prende in input una lista di valori interi, dove ogni valore intero della 
lista rappresenta la dimensione non compressa di un singolo file espressa in byte.
 
Tale funzione deve utilizzare un ciclo per iterare la lista e, per ogni dimensione non compressa, deve calcolare la dimensione 
compressa di tale file espressa come float (ovvero deve calcolare l' 80% della dimensione non compressa), calcolare il numero di blocchi 
(arrotondato al numero intero più vicino) da 512 byte necessari per la memorizzazione, al fine di determinare se il file compresso 
può essere salvato nello spazio rimanente nel supporto di memorizzazione o meno.
...
più grande di quellirimasti disponibili sul supporto di memorizzazione. In tal caso, la funzione deve avvisare l'utente che lo 
spazio disponibile sul supporto di 
...
def memorizza_file(files: list[int]) -> None:
    ...
    for file_size in files:
        dimensione_compressa = file_size * 0.8  # Dimensione compressa al 80%
        blocchi_necessari = round(dimensione_compressa / 512)  # Calcola il numero di blocchi necessari
        
        if blocchi_necessari <= spazio_totale_blocchi:
            spazio_totale_blocchi -= blocchi_necessari
            print(f"File di {file_size} byte compresso in {dimensione_compressa} byte e memorizzato. Blocchi usati: {blocchi_necessari}. Blocchi rimanenti: {spazio_totale_blocchi}.")
        else:
            print(f"Non è possibile memorizzare il file di {file_size} byte. Spazio insufficiente.")
            break

# Esempio di utilizzo
files = [1100, 99999, 2048, 3072, 34235, 43434, 434, 55, 1, 100000]
